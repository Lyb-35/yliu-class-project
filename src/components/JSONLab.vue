...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>
        Company: {{ bookstores.name.split(' ')[0] }}
      ...
      <ul>
        <li v-for="(type, count) in bookstores.storeTypes" :key="type">{{ type }}: {{ count }}</li>
      </ul>
      </p>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="{ highlight: author.name === 'Jane Austen' }"
        >
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
...
const austen = computed(() => authors.find((author) => author.id == '00001'))
</script>
